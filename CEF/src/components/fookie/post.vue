...
export default {
    ...
    mounted: async function () {
        if (this.selectedId) {
            this.body = this.$store.state[this.model].pool.find(
                (m) => m.id === this.selectedId
            );
    ...
    methods: {
        ...
        edit: async function () {
            this.dialog = false;         
            let body = JSON.parse(JSON.stringify(this.patchBody));
            let model = this.model.name;
            await this.$store.dispatch("api", {
                method: "patch",
                model,
            ...
            <v-card-text v-for="(field, i) in fields" :key="i">
                <v-row v-if="field[$store.state.self.user.type].write">
                    ...
                    <v-autocomplete
                        ...
                        :label="
                            relationModel(field.relation.model).schema[
                                relationModel(field.relation.model).display
                            ].label
                        "
                        clearable
                        item-value="id"
